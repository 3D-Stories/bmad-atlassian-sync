...
**Goal:** Bidirectional sync between BMAD .md artifacts and Jira Cloud / Confluence Cloud.
...
<step goal="Check whether Atlassian sync is configured and available">
  ...
  <check if="atlassian_sync.enabled == true AND credentials missing">
    <action>Set `sync_available = false`</action>
    <action>Warn: "Atlassian sync is enabled in config but credentials are missing from .env — skipping sync"</action>
  </check>
</step>
</initialization>
...
The following operations are available as standalone commands or triggered by agent critical actions. Each operation checks `sync_available` before executing.
